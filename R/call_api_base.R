...
#' @importFrom utils URLencode
#' @noRd
call_api_base2 <- function(sp_names, api_endpoint, loading_text, parse_function, request_method = "GET", body = NULL, reorder_result=TRUE) {
  ...
  for (i in seq_along(unique_sp_names)) {
    ...
    response <- make_request(
      ...
      body = if (!is.null(body)) {
        if (is.function(body)) body(unique_sp_names[i]) else body
      } else NULL
    )
