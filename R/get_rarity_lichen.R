...
#'
#' @examples
#' get_rarity_lichen(c("Cetraria ericetorum Opiz", "Lecanora ciliata"))
#'
#' @import utils
#' @import httr
#' @import jsonlite
#'
#' @export
get_rarity_lichen <-function(sp_names) {
  ...
  for (i in 1:length(unique_sp_names)) {
    ...
    while (!success) {
      ...
      url <- "https://italic.units.it/api/v1/rarity/"
    ...
    rarity <- data[2]
    rarity <- as.data.frame(rarity$`commoness-rarity status`)
    warnings <-  as.data.frame(data[3])
    
    result <- cbind(input,rarity,warnings)
